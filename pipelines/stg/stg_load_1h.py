...
# Load raw data
RAW_PATH = "./data/01_raw/entsoe/Load.csv"
...
df["start_time"] = pd.to_datetime(
    df["start_time_str"], format="%d/%m/%Y %H:%M", errors="coerce"
)
df["end_time"] = pd.to_datetime(
    df["end_time_str"], format="%d/%m/%Y %H:%M", errors="coerce"
)
...
df["Actual Total Load (MW)"] = pd.to_numeric(
    df["Actual Total Load (MW)"], errors="coerce"
)
...
price_cols = ["Actual Total Load (MW)", "Day-ahead Total Load Forecast (MW)"]

df_1h = df[price_cols].resample("h", label="left", closed="left").mean()
...
OUTPUT_PATH = Path(f"./data/02_stg/entsoe/load_1h/ingested_at={ingested_at_st}")
OUTPUT_PATH.mkdir(parents=True, exist_ok=True)
OUTPUT_PATH = OUTPUT_PATH / "Load_1h.csv"
